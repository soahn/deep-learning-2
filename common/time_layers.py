...
class LSTM:
    def __init__(self, Wx, Wh, b):
        ...
        ----------
        Wx: weight parameter of input x(4 weights included)
        Wh: parameter of hidden h(4 weights included)
        b: deflection（4 deflections included）
        '''
        self.params = [Wx, Wh, b]
        self.grads = [np.zeros_like(Wx), np.zeros_like(Wh), np.zeros_like(b)]
        self.cache = None
...
class TimeSoftmaxWithLoss:
    ...
    def forward(self, xs, ts):
        ...
        if ts.ndim == 3:  # if answer label is one-hot vector
            ts = ts.argmax(axis=2)
        ...
        ls *= mask  # set loss of data including ignore_label to zero
        loss = -np.sum(ls)
        loss /= mask.sum()
    ...
    def backward(self, dout=1):
        ...
        dx *= dout
        dx /= mask.sum()
        dx *= mask[:, np.newaxis]  # set gradient of data including ignore_label to zero
...
# ====================================================================== #
# Further More:(not included in the book)
# TimeSigmoidWithLoss: Sigmoid for time data + loss class
# GRU: GRU class
# TimeGRU: GRU class for time data
# BiTimeLSTM: Two-sided LSTM class
# Simple_TimeSoftmaxWithLoss：Simple TimeSoftmaxWithLoss class
# Simple_TimeAffine: Simple TimeAffine class
# ====================================================================== #
...
class GRU:
    def __init__(self, Wx, Wh):
        ...
        Parameters
        ----------
        Wx: weight parameter of input x(3 weights included)
        Wh: weight parameter of hidden h(3 weights included)
        '''
